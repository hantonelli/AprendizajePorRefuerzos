...
class DuelingDQN:
    ...
    def replay(self):
        ...
        q_state_action_values = self.model(state_batch).gather(1, action_batch)
        ...
        with torch.no_grad():
            # We use the current q to calculate Q(s', a'), and then get the action with higher value
            # argmax->a' Q(s', a')
            # argmax(1) selects choose the max from the value column
            # the [0] that's the value out of the returned array

            # Simple Q
            # L = E min [(  (r + gamma * max a' Qt(s',a'))           - Qc(s,a)  ) **2 ]
            # Double Q
            # L = E min [(  (r + gamma * Qt(s', max a' Qc(s',a')))   - Qc(s,a)  ) **2 ]
            # Sacamos el max de Qt y remplazamos a' por Qc(s',a')

            # Notation
            #    expected_state_action_values    q_next_action
            #             |  t_next_state_action_values   |       y=q_state_action_values
            #             |               |               |                |
            # L = E min [ (  (r + gamma * Qt(s', max a' Qc(s',a')))   - Qc(s,a)  ) **2 ]


            q_next_action = self.model(non_final_next_states).argmax(1)
            tmp = self.target(non_final_next_states)
            tmp2 = tmp[torch.arange(tmp.size(0)), q_next_action]

            t_next_state_action_values = torch.zeros(self.batch_size, device=self.device)
            t_next_state_action_values[non_final_mask] = tmp2

            # Compute the expected Q values
            expected_state_action_values = reward_batch + self.gamma * t_next_state_action_values

        # Compute loss
        loss = self.loss_fn(q_state_action_values, expected_state_action_values.unsqueeze(1))
...
if __name__ == '__main__':
    ...
    debug = True
    agent = AdvantageDQN(gym.make('CartPole-v0'), debug=debug)
